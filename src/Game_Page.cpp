...
void Game_Page::render(SDL_Renderer *renderer)
{
    ...
    while (cur_bul_index < main_player.bul_list.size())
    {
        ...
        {
            SDL_RenderCopyEx(renderer, main_player.bullet, NULL, &(main_player.bul_list[cur_bul_index].Position_E), main_player.bul_list[cur_bul_index].fire_angle, NULL, SDL_FLIP_NONE);
            cur_bul_index++;
        }
    }
